...
N_STEPS_PER_RUN = 6000       # default; can be overridden by --steps
AVG_START_STEP = 1500        # default; can be overridden by --avg-start
N_REPLICAS = 2               # default; can be overridden by --replicas
BASE_RANDOM_SEED = 24680
SMOOTH_WINDOW = 11           # odd integer; mild smoothing for publication-quality curves
...
def postprocess_couette_profiles(x, u, t, rho,
    ...
    The symmetry projection is applied only after the run. It suppresses
    Monte-Carlo asymmetry and removes the spurious centerline wiggle without
    changing the solver or the reported timings.
    """
...
def plot_profile_with_band(kn_value, x, y_phys_mean, y_phys_std, y_ml_mean, y_ml_std, ylabel, stem):
    ...
    ax.fill_between(xL, y_phys_mean - y_phys_std, y_phys_mean + y_phys_std,
                    color="tab:blue", alpha=0.16, linewidth=0.0, label="Physics band")
    ...
    ax.fill_between(xL, y_ml_mean - y_ml_std, y_ml_mean + y_ml_std,
                    color="tab:red", alpha=0.14, linewidth=0.0, label="ML band")
...
def save_profile_tables(kn_value, x, bundle):
    """Save the numerical data used in each PDF plot."""
    ...
    for key in ("velocity", "temperature", "density"):
        ...
        table = np.column_stack([x, xL,
                                 arr["phys_mean"], arr["phys_std"],
                                 arr["ml_mean"], arr["ml_std"]])
...
def run_one_solver(mode, replica_id, case_seed,
                   current_rho_in, current_t_in, current_theta0_in,
                   current_w_particle, current_dt, model_assets_gpu=None):
    """Run one replica of either the full physics solver or the native ML solver."""
    cp.random.seed(case_seed)
    ...
    for nt in range(1, N_STEPS_PER_RUN + 1):
        ...
        if nt % 1000 == 0:
            print(f"\r  {mode.upper()} replica {replica_id+1}/{N_REPLICAS} step {nt}/{N_STEPS_PER_RUN}...", end="")

        p_data[9][:] = p_data[0]
        p_data[10][:] = p_data[1]
        p_data[0][:] = p_data[9] + p_data[3] * current_dt

        apply_boundary_couette_cupy(p_data, LX, current_dt,
                                    UW1_BASE, UW2_BASE,
...
def run_single_comparison(kn_factor, base_rho, base_temp, model_assets_gpu):
    ...
    # Common-random-number pairing: same seed for physics and ML in each replica.
    # This reduces stochastic differences in the comparison plots.
    case_seed_offset = int(round(kn_value * 10000.0)) * 100

    for replica_id in range(N_REPLICAS):
        case_seed = BASE_RANDOM_SEED + case_seed_offset + replica_id

        phys_avg, t_phys = run_one_solver("physics", replica_id, case_seed,
                                          current_rho_in, current_t_in,
                                          current_theta0_in, current_w_particle,
                                          current_dt, model_assets_gpu)
        ...
        ml_avg, t_ml = run_one_solver("ml", replica_id, case_seed,
                                      current_rho_in, current_t_in,
                                      current_theta0_in, current_w_particle,
                                      current_dt, model_assets_gpu)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fast publication-quality Couette FP-vs-ML comparison")
    parser.add_argument("--kns", default="1.0,0.7", help="Comma-separated Kn values, e.g. 1.0,0.7")
    parser.add_argument("--ppc", type=int, default=3000, help="Particles per cell")
    parser.add_argument("--steps", type=int, default=6000, help="Total steps per replica")
    parser.add_argument("--avg-start", type=int, default=1500, help="Start averaging after this step")
    parser.add_argument("--replicas", type=int, default=2, help="Number of paired replicas")
    parser.add_argument("--smooth-window", type=int, default=11, help="Odd moving-average window for final plots")
    parser.add_argument("--seed", type=int, default=24680, help="Base RNG seed")
    parser.add_argument("--fig-dir", default="couette_figures_pdf", help="Directory for PDF figures")
    parser.add_argument("--data-dir", default="couette_profile_data", help="Directory for CSV profile data")
...
def main():
    ...
    print("=" * 72)
    print("  Fast publication-quality Couette comparison: PHYSICS vs. NATIVE ML")
    print("=" * 72)
    print(f"  GPU: {cp.cuda.runtime.getDeviceProperties(0)['name']}")
    print(f"  Nx: {NX}, particles per cell: {PARTICLES_PER_CELL_TARGET}, total particles: {NP}")
    print(f"  Steps per replica: {N_STEPS_PER_RUN}, averaging starts at: {AVG_START_STEP}")
    print(f"  Replicas: {N_REPLICAS}, smooth window: {SMOOTH_WINDOW}")
    print(f"  Couette symmetry projection in postprocess: {ENFORCE_SYMMETRY}")
