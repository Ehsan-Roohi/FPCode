...
N_SIMULATION_RUNS = 1  # retained for compatibility
N_STEPS_PER_RUN = 20000      # default; can be changed by --steps
AVG_START_STEP = 5000        # default; can be changed by --avg-start
N_REPLICAS = 1               # default; can be changed by --replicas
BASE_RANDOM_SEED = 24680
SMOOTH_WINDOW = 9            # postprocess smoothing only
ENFORCE_SYMMETRY = True      # postprocess only: Couette symmetry cleanup
FIG_DIR = Path("couette_figures_pdf_safe")
DATA_DIR = Path("couette_profile_data_safe")
SAVE_PNG_COPY = False
EPSILON = 1e-30 
...
def postprocess_couette_profiles(x, u, t, rho, smooth_window=SMOOTH_WINDOW, enforce_symmetry=ENFORCE_SYMMETRY):
    """
    Postprocess one Couette profile for paper-quality plotting.
    This does not change the solver. It only cleans the final plotted profiles.
    """
...
def plot_profile_with_band(kn_value, x, y_phys_mean, y_phys_std, y_ml_mean, y_ml_std, ylabel, stem):
    FIG_DIR.mkdir(parents=True, exist_ok=True)
    fig, ax = _make_publication_axes(ylabel)
    xL = np.asarray(x, dtype=np.float64) / LX

    ax.plot(xL, y_phys_mean, color="tab:blue", linewidth=2.2, label="Physics")
    if np.nanmax(np.abs(y_phys_std)) > 0:
        ax.fill_between(xL, y_phys_mean-y_phys_std, y_phys_mean+y_phys_std,
                        color="tab:blue", alpha=0.16, linewidth=0.0)

    ax.plot(xL, y_ml_mean, color="tab:red", linestyle="--", linewidth=2.2, label="ML")
    if np.nanmax(np.abs(y_ml_std)) > 0:
        ax.fill_between(xL, y_ml_mean-y_ml_std, y_ml_mean+y_ml_std,
                        color="tab:red", alpha=0.14, linewidth=0.0)
...
def save_profile_tables(kn_value, x, bundle):
    ...
    for key in ("velocity", "temperature", "density"):
        ...
        table = np.column_stack([x, xL, arr["phys_mean"], arr["phys_std"], arr["ml_mean"], arr["ml_std"]])
...
def load_native_model_to_gpu(npz_filename):
    ...
    try:
        ...
        print("All model parameters (weights, scalers) transferred to GPU.", flush=True)
        print(f"  W1 on GPU, shape: {model_assets_gpu['W1'].shape}", flush=True)
        print(f"  X_mean on GPU, shape: {model_assets_gpu['X_mean'].shape}", flush=True)
        return model_assets_gpu
    except FileNotFoundError:
        print(f"Error: '{npz_filename}' not found.", flush=True)
        print("Please run the 4-layer extract_params.py / conversion script first.", flush=True)
        return None
...
def run_one_solver(mode, replica_id, current_rho_in, current_t_in, current_theta0_in, current_w_particle, current_dt, model_assets_gpu=None):
    # Same seed for physics and ML in each replica: paired random-number setup for cleaner comparisons.
    seed = BASE_RANDOM_SEED + replica_id
    cp.random.seed(seed)
    np.random.seed(seed)
    ...
    for nt in range(1, N_STEPS_PER_RUN + 1):
        ...
        if nt == 1 or nt % 50 == 0:
            ...
            print(
                f"\n  {mode.upper()} replica {replica_id+1}/{N_REPLICAS} "
                f"step {nt}/{N_STEPS_PER_RUN} | elapsed={elapsed_now/60.0:.1f} min "
        ...
        apply_boundary_couette_cupy(p_data, LX, current_dt, UW1_BASE, UW2_BASE,
                                    THETAW1_GLOBAL, THETAW2_GLOBAL)
    ...
    print(f"\nFinished {mode.upper()} replica {replica_id+1}/{N_REPLICAS} in {elapsed:.2f} s", flush=True)
...
def run_single_comparison(kn_factor, base_rho, base_temp, model_assets_gpu):
    ...
    print("\n" + "="*72, flush=True)
    print(f"Starting comparison for Kn = {kn_str}", flush=True)
    print(f"rho_in = {current_rho_in:.4e}, particle weight = {current_w_particle:.4e}", flush=True)
    print(f"Replicas = {N_REPLICAS}, averaging starts at step {AVG_START_STEP}", flush=True)
    ...
    for replica_id in range(N_REPLICAS):
        phys_avg, t_phys = run_one_solver("physics", replica_id, current_rho_in, current_t_in,
                                          current_theta0_in, current_w_particle, current_dt,
                                          model_assets_gpu)
        x_proc, u_proc, t_proc, rho_proc = postprocess_couette_profiles(
            phys_avg["pos"], phys_avg["U"][:, 1], phys_avg["T"], phys_avg["rho"]
        )
        phys_u_list.append(u_proc); phys_t_list.append(t_proc); phys_rho_list.append(rho_proc)
        phys_times.append(t_phys)
        x_ref = x_proc

        ml_avg, t_ml = run_one_solver("ml", replica_id, current_rho_in, current_t_in,
                                      current_theta0_in, current_w_particle, current_dt,
                                      model_assets_gpu)
        x_proc_ml, u_proc_ml, t_proc_ml, rho_proc_ml = postprocess_couette_profiles(
            ml_avg["pos"], ml_avg["U"][:, 1], ml_avg["T"], ml_avg["rho"]
        )
        ml_u_list.append(u_proc_ml); ml_t_list.append(t_proc_ml); ml_rho_list.append(rho_proc_ml)
    ...
    phys_u = np.vstack(phys_u_list); phys_t = np.vstack(phys_t_list); phys_rho = np.vstack(phys_rho_list)
    ml_u = np.vstack(ml_u_list); ml_t = np.vstack(ml_t_list); ml_rho = np.vstack(ml_rho_list)
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description="Safe publication-quality Couette FP-vs-ML comparison")
    parser.add_argument("--kns", default="1.0", help="Comma-separated Kn values, e.g. 1.0 or 1.0,0.7")
    parser.add_argument("--ppc", type=int, default=30000, help="Particles per cell. Keep near training/publication regime for ML.")
    parser.add_argument("--steps", type=int, default=20000, help="Total steps per replica")
    parser.add_argument("--avg-start", type=int, default=5000, help="Start averaging after this step")
    parser.add_argument("--replicas", type=int, default=1, help="Number of paired replicas")
    parser.add_argument("--smooth-window", type=int, default=9, help="Odd moving-average window for final plots")
    parser.add_argument("--seed", type=int, default=24680, help="Base RNG seed")
    parser.add_argument("--fig-dir", default="couette_figures_pdf_safe", help="Directory for PDF figures")
    parser.add_argument("--data-dir", default="couette_profile_data_safe", help="Directory for CSV profile data")
    ...
    parser.add_argument("--no-symmetry", action="store_true", help="Disable Couette symmetry projection in postprocess")
    return parser.parse_args()


def main():
    global PARTICLES_PER_CELL_TARGET, NP, N_STEPS_PER_RUN, AVG_START_STEP
    global N_REPLICAS, BASE_RANDOM_SEED, SMOOTH_WINDOW, ENFORCE_SYMMETRY
    global FIG_DIR, DATA_DIR, SAVE_PNG_COPY

    args = parse_args()
    PARTICLES_PER_CELL_TARGET = int(args.ppc)
    NP = PARTICLES_PER_CELL_TARGET * NC
    N_STEPS_PER_RUN = int(args.steps)
    AVG_START_STEP = int(args.avg_start)
    N_REPLICAS = int(args.replicas)
    ...
    if AVG_START_STEP >= N_STEPS_PER_RUN:
        raise ValueError("--avg-start must be smaller than --steps")
    if N_REPLICAS < 1:
        raise ValueError("--replicas must be >= 1")
    if PARTICLES_PER_CELL_TARGET < 10000:
        print(
            "WARNING: ppc is much lower than the training/publication regime. "
            "ML profiles may become unreliable because moment noise is out-of-distribution.",
            flush=True,
        )

    print("="*72, flush=True)
    print("  Safe publication-quality Couette comparison: PHYSICS vs. NATIVE ML", flush=True)
    print("="*72, flush=True)
    print(f"  GPU: {cp.cuda.runtime.getDeviceProperties(0)['name']}", flush=True)
    print(f"  Nx: {NX}, particles per cell: {PARTICLES_PER_CELL_TARGET}, total particles: {NP}", flush=True)
    print(f"  Steps per replica: {N_STEPS_PER_RUN}, averaging starts at: {AVG_START_STEP}", flush=True)
    print(f"  Replicas: {N_REPLICAS}, smooth window: {SMOOTH_WINDOW}", flush=True)
    print(f"  Couette symmetry projection in postprocess: {ENFORCE_SYMMETRY}", flush=True)
    ...
    print("="*80, flush=True)
    print(f"{'Kn':<12} | {'Physics mean time (s)':<24} | {'ML mean time (s)':<18} | {'Speedup':<10}", flush=True)
    print("-"*80, flush=True)
    for kn, time_p, time_ml, speedup, _, _ in results_list:
        print(f"{kn:<12.4g} | {time_p:<24.2f} | {time_ml:<18.2f} | {speedup:<10.2f}x", flush=True)
    print("="*80, flush=True)

    print("\nGenerating separate publication-quality PDF plots...", flush=True)
    for kn_value, _, _, _, pos_data, bundle in results_list:
        plot_profile_with_band(
            kn_value, pos_data,
            bundle["velocity"]["phys_mean"], bundle["velocity"]["phys_std"],
            bundle["velocity"]["ml_mean"], bundle["velocity"]["ml_std"],
            r"$U_y$ (m/s)", "couette_velocity",
        )
        plot_profile_with_band(
            kn_value, pos_data,
            bundle["temperature"]["phys_mean"], bundle["temperature"]["phys_std"],
            bundle["temperature"]["ml_mean"], bundle["temperature"]["ml_std"],
            r"$T$ (K)", "couette_temperature",
        )
        plot_profile_with_band(
            kn_value, pos_data,
            bundle["density"]["phys_mean"], bundle["density"]["phys_std"],
            bundle["density"]["ml_mean"], bundle["density"]["ml_std"],
            r"$\rho$ (kg/m$^3$)", "couette_density",
        )
