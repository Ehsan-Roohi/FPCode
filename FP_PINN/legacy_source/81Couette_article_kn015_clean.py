...
N_STEPS_PER_RUN = 8000      # overridden by --steps
EPSILON = 1e-30

# Publication plotting / run controls; overridden by command-line arguments.
FIG_DIR = Path("couette_article_kn015_figures")
DATA_DIR = Path("couette_article_kn015_data")
SMOOTH_WINDOW = 5
SAVE_PNG_COPY = False
...
# ============================================================================
# 5. Plotting Functions: article Couette, separate PDF, no title
# ============================================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Article Couette run at fixed Kn with clean PDF plots. Original solver dynamics unchanged."
    )
    parser.add_argument("--kns", default="0.15", help="Comma-separated Kn values. For the paper Couette figure use 0.15.")
    parser.add_argument("--nx", type=int, default=100, help="Number of cells. Paper compact Couette uses 100.")
    parser.add_argument("--ppc", type=int, default=15000, help="Particles per cell. Increase for cleaner profiles.")
    parser.add_argument("--steps", type=int, default=8000, help="Total steps. Increase for longer time averaging.")
    ...
    parser.add_argument("--fig-dir", default="couette_article_kn015_figures", help="Output directory for PDF figures.")
    parser.add_argument("--data-dir", default="couette_article_kn015_data", help="Output directory for CSV profile data.")
...
def main():
    ...
    print("=" * 72, flush=True)
    print("  Article Couette comparison with clean PDF plots", flush=True)
    print("=" * 72, flush=True)
    ...
    print(f"  Smooth window: {SMOOTH_WINDOW}", flush=True)
    print("  Solver dynamics: unchanged from 75Couette2Kn(3).py", flush=True)
