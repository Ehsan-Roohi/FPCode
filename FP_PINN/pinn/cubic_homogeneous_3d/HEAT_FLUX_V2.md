...
> Historical note: this document records the v2 continuation experiments.
> Its former 5% "publication" label is superseded by `HEAT_FLUX_G0.md`, where
> 5% is the continuation gate, 2% is the primary gate, and 1% is publication.
...
The log must include finite `pde=` and `qweak=` values.  This historical v2 run
used `heat_flux_history_relative_l2 < 0.05` as its target and `0.20` only as a
smoke threshold; G0 supersedes those labels and uses the analytic history.  If
differentiating the closure is unstable on a particular GPU,
