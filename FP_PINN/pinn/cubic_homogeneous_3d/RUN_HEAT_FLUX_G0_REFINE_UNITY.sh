...
if [[ ! -d "$FP_TEST/.git" ]]; then
    echo "ERROR: refinement checkout not found: $FP_TEST" >&2
    exit 2
fi
if [[ ! -f "$FP_RESUME_ARCHIVE" ]]; then
    echo "ERROR: successful G0 archive not found: $FP_RESUME_ARCHIVE" >&2
    echo "Set FP_RESUME_ARCHIVE=/absolute/path/to/FP_PINN_STAGE2_JOB63178434_HEAT_FLUX_COMPLETE.zip" >&2
    exit 2
fi
...
sbatch \
  ...
  --output="$FP_TEST/slurm_logs/fp-g0-refine-%A_%a.out" \
  --error="$FP_TEST/slurm_logs/fp-g0-refine-%A_%a.err" \
  FP_PINN/pinn/cubic_homogeneous_3d/slurm/run_heat_flux_g0_refine_array.sbatch
