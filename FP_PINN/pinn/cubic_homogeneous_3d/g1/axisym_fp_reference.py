#!/usr/bin/env python3
"""Deterministic axisymmetric (cx, rho) finite-volume reference for the homogeneous
cubic Fokker--Planck relaxation (heat_flux case of FP_PINN/pinn/cubic_homogeneous_3d).

The initial state f0(cx) N(cy) N(cz) and the homogeneous cubic FP operator are invariant
under rotations about the x axis, so the exact solution is f(t, cx, rho),
...
noise-free reference for marginals and pointwise density.

Usage (from FP_PINN/pinn/cubic_homogeneous_3d):
    python g1/axisym_fp_reference.py --output-dir REF --nx 400 --nr 200 --L 8 --dt 2e-4
"""
...
def run(args: argparse.Namespace) -> dict:
    ...
    for n in range(1, steps + 1):
        ...
        if n % progress_every == 0:
            m = grid.moments(f)
            print(f"  t={n*dt:.3f} mass={m.mass:.10f} dm2={m.dm2:.10f} qx={m.q[0]:.8f} "
                  f"exact={0.25*np.exp(-4/3*args.nu*n*dt):.8f} pxx-pyy={m.pij[0]-m.pij[3]:+.2e} "
                  f"min f={f.min():.2e} [{time.perf_counter()-t0:.0f}s]", flush=True)
    arrays = {k: np.asarray(v) for k, v in rec.items()}
    times = arrays["time"]
    exact = 0.25 * np.exp(-4.0 / 3.0 * args.nu * times)
    qx = arrays["q"][:, 0]
    rel_l2 = float(np.linalg.norm(qx - exact) / np.linalg.norm(exact))
    rate = float(-np.dot(times[1:], np.log(qx[1:] / 0.25)) / np.dot(times[1:], times[1:]))
    ...
    metrics = {
        ...
        "qx_vs_exact_relative_l2": rel_l2, "fitted_decay_rate": rate, "exact_decay_rate": 4.0 / 3.0 * args.nu,
        ...
        "max_stress_anisotropy": float(np.max(np.abs(arrays["pij"][:, 0] - arrays["pij"][:, 3]))),
