"""Structure-preserving density ansatz for the heat-flux G1 stage.

The G0 stage represented log f with a bridge + network correction and asked a
soft penalty to keep mass, momentum and energy.  The deterministic audit of
commit 97f9ea8 (epoch-12500 checkpoint) showed that the optimiser traded those
invariants against the pointwise residual: the true drift was 3.1 % in mass
and 4.5 % in energy, the projection of the PDE residual on the invariants was
O(0.2-0.4) per unit time, and a spurious stress anisotropy p_xx - p_yy = -0.105
had developed by t = 1.  G1 removes the invariant trade-off by construction
and evaluates every integral with a deterministic quadrature.

Ansatz (all symbols as in train_stage2.py):

    log f~(t, c) = log[(1-a) f0 + a M] + t cap tanh(raw/cap) + b(t) phi3(c)
    log f (t, c) = log f~(t, c) + beta(t) . psi(c),    psi = (1, cx, |c|^2 - 3)

* f0, M, a(t) and the network correction are *unchanged* from DensityModel,
  so every G0 weight file remains loadable into ``model.base`` and can be
  used as a warm start.
* phi3(c) = cx (|c|^2 - 5) is the third Hermite (heat-flux) mode.  Its
  amplitude b(t) = t b_cap tanh(head(t)/b_cap) is produced by a tiny network of
  t alone, vanishes at t=0 (hard initial condition) and is bounded.
...
from axisym_quadrature import AxisymQuadrature  # noqa: E402
...
class QuadratureTensors:
    ...
    phi64: tf.Tensor       # [nv] float64, cx |c|^2 (raw heat-flux basis)
...
def heat_flux_mode_tf(c: tf.Tensor) -> tf.Tensor:
    """phi3(c) = cx (|c|^2 - 5), shape [n, 1], same dtype as c."""
...
# Model
# --------------------------------------------------------------------------
class HeatFluxHead(tf.keras.layers.Layer):
    """b(t) = t * cap * tanh(mlp(t / tmax) / cap); b(0) = 0 and |b| <= cap * t."""
...
class StructuredDensityModel(tf.keras.Model):
    """DensityModel + heat-flux mode + exact invariant tilt."""

    def __init__(self, base_config: BaseConfig, g1: G1Config):
        super().__init__()
        self.base_config = base_config
        self.g1 = g1
        self.base = DensityModel(base_config)
        self.head = (
            HeatFluxHead(g1.heat_flux_mode_width, g1.heat_flux_mode_cap, g1.tmax, base_config.seed)
            if g1.use_heat_flux_mode else None
        )
    ...
    def raw_log_density(self, t: tf.Tensor, c: tf.Tensor) -> tf.Tensor:
        ...
        if self.head is not None:
            log_f = log_f + self.head(t) * heat_flux_mode_tf(c)
        return log_f
...
def heat_flux_rate_residual(state: SliceState, quad: QuadratureTensors, nu: float) -> tuple[tf.Tensor, tf.Tensor]:
    """Weak-form heat-flux identity of the operator: dQx/dt + (4/3) nu Qx.

    Qx = sum_v w f cx |c|^2 (mean velocity is zero by construction), and
    dQx/dt = sum_v w f cx |c|^2 d_t log f.  Returns (Qx [nt], residual [nt]).
    This is a consequence of the cubic closure (not of the analytic answer);
    it is OFF by default in G1 and provided only for a documented ablation.
    """
    phi = quad.phi64
    qx = tf.reduce_sum(state.wf * phi[None, :], axis=1)
    dqx = tf.reduce_sum(state.wf * phi[None, :] * tf.cast(state.h_t[:, :, 0], tf.float64), axis=1)
    return qx, dqx + (4.0 / 3.0) * nu * qx
