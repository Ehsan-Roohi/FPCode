#!/usr/bin/env python3
"""Create one atomic ZIP archive plus SHA-256 sidecar for a G2 Slurm task.
...
def read_status(case_output: Path) -> tuple[str, dict]:
    ...
    if sweep.exists():
        ...
        return summary["selected_status"], {
            ...
            "stress_analytic_l2_fine": summary["selected_metrics"]["stress_analytic_l2_fine"],
    ...
    if final.exists():
        ...
        return metrics["status"], {"source": "final_weights", "stress_analytic_l2_fine": metrics["stress_analytic_l2_fine"]}
...
def main() -> None:
    ...
    if not case_output.is_dir():
        raise SystemExit(f"G2 case output does not exist: {case_output}")
    ...
    print(f"G2_ARCHIVE {archive}")
    print(f"G2_ARCHIVE_SHA256 {digest}")
    print(f"G2_ARCHIVE_STATUS {status}")
