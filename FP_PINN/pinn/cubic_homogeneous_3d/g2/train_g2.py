#!/usr/bin/env python3
"""G2 axisymmetric stress relaxation on the validated G1 structure.

The training objective is the cubic Fokker--Planck PDE residual plus the
invariant-tilt gauge penalty.  The analytic stress law is never used by the
qualification run.  It appears only in deterministic held-out evaluation.
...
3. The loss is the f-weighted mean square of the log residual *only*.  No
   ...
   qualification run.)
4. An explicit traceless second-Hermite stress mode with zero amplitude at t=0.
...
from g1.axisym_quadrature import build_quadrature  # noqa: E402
...
from evaluate_g2 import evaluate_model  # noqa: E402
...
class G2TrainConfig:
    ...
    stress_rate_weight: float = 0.0         # ablation only; 0 in the qualification run
    nu: float = 1.0
    tmax: float = 1.0
    print_every: int = 100
    checkpoint_every: int = 1_000
    seed: int = 20260901
    resume_base_weights: str | None = None  # optional compatible base-network warm start
    resume_weights: str | None = None       # G2 StructuredDensityModel weights
    evaluate_only: bool = False
    strict_gate: bool = False


def parse_args() -> G2TrainConfig:
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    defaults = G2TrainConfig(output_dir="", reference="")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--reference", required=True, help="axisym_stress_reference.py reference.npz")
    for name, field in G2TrainConfig.__dataclass_fields__.items():
    ...
    return G2TrainConfig(**vars(args))


def base_config_from(config: G2TrainConfig) -> BaseConfig:
    return BaseConfig(
        case="stress", output_dir=config.output_dir, reference=config.reference,
...
def g2_config_from(config: G2TrainConfig) -> G2Config:
    return G2Config(
        use_stress_mode=config.use_stress_mode, stress_mode_cap=config.stress_mode_cap,
        stress_mode_width=config.stress_mode_width, tilt_newton_steps=config.tilt_newton_steps,
...
def make_train_step(model: StructuredDensityModel, optimizer, config: G2TrainConfig, base_quad: QuadratureTensors):
    ...
    def train_step() -> dict[str, tf.Tensor]:
        ...
        with tf.GradientTape() as tape:
            ...
            if config.stress_rate_weight > 0.0:
                _, rate_residual = stress_rate_residual(state, quad, config.nu)
                rate_loss = tf.cast(tf.reduce_mean(tf.square(rate_residual / (2.0 * config.nu * 0.9))), tf.float32)
            else:
                rate_loss = tf.constant(0.0, tf.float32)
            total = pde_loss + config.tilt_penalty_weight * tilt_loss + config.stress_rate_weight * rate_loss
            tf.debugging.assert_all_finite(total, "Non-finite G2 training loss")
        ...
        for gradient in gradients:
            tf.debugging.assert_all_finite(gradient, "Non-finite G2 gradient")
        ...
        return {
            "total": total, "pde": pde_loss, "tilt": tilt_loss, "stress_rate": rate_loss, "grad_norm": grad_norm,
...
def portable_reload_check(model: StructuredDensityModel, weights: Path, base: BaseConfig, g2: G2Config) -> float:
    ...
    reloaded = StructuredDensityModel(base, g2)
...
def main() -> None:
    ...
    g2 = g2_config_from(config)
    full_config: dict[str, Any] = {**asdict(config), **asdict(g2), "base_config": asdict(base)}
    ...
    if config.resume_weights:
        ...
        print(f"Loaded G2 weights: {config.resume_weights}", flush=True)
    ...
    final_weights = output / "g2_final.weights.h5"
    model.save_weights(final_weights)
    model.base.save_weights(output / "g2_final_base_only.weights.h5")
    reload_linf = portable_reload_check(model, final_weights, base, g2)
    ...
    print("G2_FINAL_METRICS " + json.dumps(metrics, sort_keys=True), flush=True)
    print(f"G2_STATUS {metrics['status']}", flush=True)
