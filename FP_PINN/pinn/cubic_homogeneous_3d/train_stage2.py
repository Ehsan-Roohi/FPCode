...
def evaluate(model: DensityModel, config: Config, output: Path) -> dict[str, Any]:
    ...
    if case_has_heat_flux(config.case):
        ...
        exact_qx0=initial_heat_flux_qx(config.case)
        analytic_qx=analytic_heat_flux_history(times,qx0=exact_qx0,nu=config.nu)
        ...
        model_decay_rate=fit_decay_rate(times,model_q[:,0],qx0=exact_qx0)
        particle_decay_rate=fit_decay_rate(times,ref_q[:,0],qx0=exact_qx0)
    ...
    if has_heat_flux and has_stress:
        relevant_error = max(stress_error, analytic_heat_error)
    ...
    if has_heat_flux:
        ...
        checks["heat_flux_analytic_history"] = bool(
            heat_flux_gates is not None and heat_flux_gates["continuation_pass"]
        )
    if has_stress:
        checks["stress_history_relative_l2"] = stress_error <= 0.15
    ...
    if has_heat_flux:
        ...
        metrics.update({
            ...
            "publication_target_passed":bool(
                heat_flux_gates["continuation_pass"] and all(checks.values())
            ),
        })
