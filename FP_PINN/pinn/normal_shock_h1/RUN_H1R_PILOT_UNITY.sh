...
sha256sum "$OUT"/* > "$OUT/SHA256SUMS"; A="$ROOT/FP_PINN_H1R_JOB${SLURM_JOB_ID}_M${MACH//./p}_COMPLETE.zip"; (cd "$HERE/outputs" && zip -qr "$A" "$(basename "$OUT")"); sha256sum "$A" > "$A.sha256"; echo "H1R_OUTPUT=$OUT"; echo "H1R_ARCHIVE=$A"; exit "$RC"
