...
OUT="$HERE/outputs/h1r_m${MACH//./p}_${SLURM_JOB_ID}"; cd "$HERE"; python -m unittest -v test_shock_physics.py test_fp_operator.py; ARGS=(--mach "$MACH" --epochs "${FP_H1R_EPOCHS:-3000}" --output "$OUT"); if [[ -n "${FP_H1R_RESTART:-}" ]]; then [[ -f "$FP_H1R_RESTART" ]] || { echo "ERROR restart not found: $FP_H1R_RESTART" >&2; exit 2; }; ARGS+=(--restart "$FP_H1R_RESTART"); fi; set +e; python train_h1r.py "${ARGS[@]}"; RC=$?; set -e
