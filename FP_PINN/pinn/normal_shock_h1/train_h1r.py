...
def main():
 ...
 def tilt(fraw):
  ...
  for _ in range(a.tilt_steps):
   f=fraw*tf.exp(tf.clip_by_value(tf.einsum("xi,xvri->xvr",beta,psif),-20.,20.)); wf=f*measure; mom=tf.einsum("xvr,xvri->xi",wf,psif); jac=tf.einsum("xvr,xvri,xvrj->xij",wf,psif,psif); delta=tf.linalg.solve(jac+1e-10*eye[None],(target[None]-mom)[...,None])[...,0]; beta=(beta+.8*delta)*mask
 ...
 gates={"finite":bool(np.isfinite(f).all()),"positive":bool(f.min()>0),"boundary":be<2e-3,"mass_flux":spreads["mass"]<2e-3,"momentum_flux":spreads["momentum"]<2e-3,"energy_flux":spreads["energy"]<2e-3,"residual":rr<.20}; status="PILOT_PASS" if all(gates.values()) else "NO_GO"; metrics={"stage":"H1R_STRUCTURAL_FLUX_PILOT","status":status,"mach":a.mach,"residual_relative_rms":rr,"flux_relative_spreads":spreads,"boundary_relative_error":be,"min_distribution":float(f.min()),"max_abs_heat_flux":float(np.max(abs(qx))),"max_stress_anisotropy":float(np.max(abs(pxx-pp))),"tilt_beta_max":float(np.max(abs(beta.numpy()))),"epochs":a.epochs,"elapsed_seconds":time.time()-start,"gates":gates,"claim":"Structural-conservation pilot; held-out reference qualification remains required."}; (out/"metrics.json").write_text(json.dumps(metrics,indent=2)+"\n"); np.savetxt(out/"profiles.csv",np.column_stack((x,rho,u,T,pxx,pp,qx,physical)),delimiter=",",header="x,rho,u,temperature,pxx,pperp,qx,mass_flux,momentum_flux,energy_flux",comments=""); np.savez_compressed(out/"h1r_solution.npz",x=x,vx=vx,vr=vr,f=f,residual=res,rho=rho,u=u,temperature=T,pxx=pxx,pperp=pp,qx=qx); model.save_weights(out/"h1r.weights.h5"); np.savetxt(out/"history.csv",hist,delimiter=",",header="epoch,loss,pde,tilt_error",comments="")
