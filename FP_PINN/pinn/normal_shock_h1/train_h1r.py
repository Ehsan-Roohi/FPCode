...
def main():
 ...
 model=tf.keras.Sequential([tf.keras.layers.Input((3,))]+[tf.keras.layers.Dense(a.width,activation="tanh") for _ in range(a.depth)]+[tf.keras.layers.Dense(1,kernel_initializer="zeros")]); restart=Path(a.restart) if a.restart else None
 if restart is not None:
  if not restart.is_file(): raise FileNotFoundError(f"restart weights not found: {restart}")
  model.load_weights(restart); print(f"H1R_RESTART={restart}",flush=True)
 opt=tf.keras.optimizers.Adam(tf.keras.optimizers.schedules.ExponentialDecay(a.lr,max(a.epochs//2,1),.3,staircase=True))
 ...
 def tilt(fraw):
  ...
  for _ in range(a.tilt_steps):
   f=fraw*tf.exp(tf.clip_by_value(tf.einsum("xi,xvri->xvr",beta,psif),-20.,20.)); wf=f*measure; mom=tf.einsum("xvr,xvri->xi",wf,psif); jac=tf.einsum("xvr,xvri,xvrj->xij",wf,psif,psif); delta=tf.linalg.solve(jac+1e-10*eye[None],(target[None]-mom)[...,None])[...,0]; delta=tf.clip_by_value(delta,-a.tilt_step_limit,a.tilt_step_limit); damping=a.tilt_damping if _ < a.tilt_full_start else 1.; beta=beta+damping*delta
