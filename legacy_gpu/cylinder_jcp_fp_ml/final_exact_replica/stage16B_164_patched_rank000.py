# Auto-generated Stage16B collector import copy.
# Original source is not modified.
# Source: 164CylFPAdaptive.py
# Generated: Tue Jun  9 19:59:10 2026
...
def compute_adaptive_coefficients_kernel(packed_central, N_arr, adaptation_map, tau, rho, c_C, c_Gamma, c_Beta, nx, ny):
    ...
    # --- FORTRAN-STYLE STABILITY FORMULA ---
    # به جای دترمینان، از مجموع مربعات استفاده می‌کنیم (مثل کد cavity.FOR)
    # این روش در شوک‌های قوی پایدارتر است.
    
    # 1. محاسبه انحرافات تانسور تنش نرمالیزه شده
    # Pij = u2_ij - (1/3)*tr_u2 * delta_ij
    dev_xx = u2_0 - tr_u2/3.0
    dev_yy = u2_1 - tr_u2/3.0
    dev_zz = u2_2 - tr_u2/3.0
    dev_xy = u2_3
    dev_xz = u2_4
    dev_yz = u2_5
    
    # 2. مجموع مربعات (Sum of Squares)
    # PGLAM = (Pxx^2 + Pyy^2 + Pzz^2 + 2*Pxy^2 + 2*Pxz^2 + 2*Pyz^2)
    sum_sq = (dev_xx**2 + dev_yy**2 + dev_zz**2 + 2.0*(dev_xy**2 + dev_xz**2 + dev_yz**2))
    
    # 3. فرمول نهایی Lambda (طبق کد فرترن)
    # PGLAM = -PGLAM * (0.5/tau) / (DM2pg^3.5)
    # توجه: DM2pg همان tr_u2 است.
    
    denom = tau_eff * (tr_u2**3.5) + 1e-30
    
    # ضریب 0.5 در کد فرترن وجود دارد: ((Ppg/vispg)*0.5)
    Lambda = -0.5 * sum_sq / denom
    
    # لیمیتر بسیار نرم برای جلوگیری از NaN (فقط در شروع)
    if abs(Lambda) > 1e10: Lambda = 0.0
