...
# -------------------------------------------------------------------
# Safety projection for shock-dominated cylinder ML deployment.
# Used only in ML mode. It prevents out-of-manifold DNN inputs/outputs
# from producing nonphysical coefficients and temperature blow-up.
# -------------------------------------------------------------------
SAFETY_ENABLE = True

# Clip standardized DNN inputs and outputs.
SAFETY_INPUT_CLIP_Z = 6.0
SAFETY_OUTPUT_CLIP_Z = 3.0

# Thermodynamic bounds.
SAFETY_T_FLOOR = 10.0
SAFETY_T_CEIL = 6000.0

SAFETY_RHO_FLOOR = 1.0e-10
SAFETY_RHO_CEIL  = 5.0e-3

# Absolute guard after unscaling coefficients.
SAFETY_COEFF_ABS_CLIP = 1.0e6

# -------------------------------------------------------------------
# Entropy-Stabilized ML closure.
# This is NOT a hybrid fallback.  No deterministic FP coefficients are
# restored.  The learned closure remains active everywhere, but its
# high-order correction is entropy-limited using local nonequilibrium
# indicators built from stress and heat-flux moments.
# -------------------------------------------------------------------
ESML_ENABLE = True

# Sensor threshold.  Larger means limiter activates only in stronger
# nonequilibrium/shock states.
ESML_SENSOR0 = 0.10

# Limiter strength for the C_ij block and Gamma_i block.
# Gamma/heat-flux block usually needs stronger limiting in shock layers.
ESML_BETA_C = 5.0
ESML_BETA_G = 20.0

# Minimum limiter values.  These keep the ML correction active everywhere.
# Set to 0 only if you want a very aggressive limit, but it is better
# to keep small positive values to avoid making the closure degenerate.
ESML_LAMBDA_C_MIN = 0.02
ESML_LAMBDA_G_MIN = 0.002

# Sensor caps to prevent single noisy cells from dominating.
ESML_SENSOR_CAP = 50.0

# Optional absolute coefficient guard after entropy limiting.
ESML_COEFF_ABS_CLIP = 1.0e8


# Additional trust-region damping for shock-dominated cylinder deployment.
# These factors reduce entropy overshoot caused by out-of-manifold ML closure coefficients.
SAFETY_C_DAMP = 1.0
SAFETY_GAMMA_DAMP = 1.0
...
# -------------------------------------------------------------------
# SAFETY KERNELS FOR ML CYLINDER DEPLOYMENT
# -------------------------------------------------------------------

@cuda.jit
def safety_project_moments_kernel(rho, T, stress, heat):
    idx = cuda.grid(1)
    if idx >= rho.shape[0]:
        return
    ...
    # Density projection.
    if not (rv >= SAFETY_RHO_FLOOR and rv <= SAFETY_RHO_CEIL):
        if rv < SAFETY_RHO_FLOOR:
            rho[idx] = SAFETY_RHO_FLOOR
        elif rv > SAFETY_RHO_CEIL:
            rho[idx] = SAFETY_RHO_CEIL
        else:
            rho[idx] = SAFETY_RHO_FLOOR

    # Temperature projection.
    if not (Tv >= SAFETY_T_FLOOR and Tv <= SAFETY_T_CEIL):
        if Tv < SAFETY_T_FLOOR:
            T[idx] = SAFETY_T_FLOOR
        elif Tv > SAFETY_T_CEIL:
            T[idx] = SAFETY_T_CEIL
        else:
            T[idx] = SAFETY_T_FLOOR

    # Remove non-finite stress/heat entries.
    for j in range(6):
        v = stress[idx, j]
        if not (v > -1.0e300 and v < 1.0e300):
            stress[idx, j] = 0.0
...
@cuda.jit
def safety_project_coefficients_kernel(c_C, c_Gamma, c_Beta, tau_arr, rho, T):
    idx = cuda.grid(1)
    if idx >= rho.shape[0]:
        return
    ...
    # If state is outside safe thermodynamic range, use conservative zero
    # high-order correction in that cell.
    invalid_state = not (
        rv >= SAFETY_RHO_FLOOR and rv <= SAFETY_RHO_CEIL and
        Tv >= SAFETY_T_FLOOR and Tv <= SAFETY_T_CEIL
    )
    ...
    # Clip non-finite or extreme coefficients.
    for j in range(6):
        v = c_C[idx, j]
        if not (v >= -SAFETY_COEFF_ABS_CLIP and v <= SAFETY_COEFF_ABS_CLIP):
            if v > 0.0:
                c_C[idx, j] = SAFETY_COEFF_ABS_CLIP
            elif v < 0.0:
                c_C[idx, j] = -SAFETY_COEFF_ABS_CLIP
            else:
                c_C[idx, j] = 0.0
    ...
    for j in range(3):
        ...
        if not (v >= -SAFETY_COEFF_ABS_CLIP and v <= SAFETY_COEFF_ABS_CLIP):
            if v > 0.0:
                c_Gamma[idx, j] = SAFETY_COEFF_ABS_CLIP
            elif v < 0.0:
                c_Gamma[idx, j] = -SAFETY_COEFF_ABS_CLIP
            else:
                c_Gamma[idx, j] = 0.0
...
@cuda.jit
def safety_damp_coefficients_kernel(c_C, c_Gamma):
    idx = cuda.grid(1)
    if idx >= c_C.shape[0]:
        return

    for j in range(6):
        c_C[idx, j] = SAFETY_C_DAMP * c_C[idx, j]

    for j in range(3):
        c_Gamma[idx, j] = SAFETY_GAMMA_DAMP * c_Gamma[idx, j]



# -------------------------------------------------------------------
# ENTROPY-STABILIZED ML LIMITER KERNEL
# -------------------------------------------------------------------

@cuda.jit
def esml_entropy_limiter_kernel(
    c_C, c_Gamma,
    rho, T, stress, heat, tree_children
):
    idx = cuda.grid(1)
    ...
    # Apply only to leaf cells.
    if tree_children[idx, 0] != -1:
        return
    ...
    if rv <= 0.0 or Tv <= 0.0:
        # Do not try to correct an invalid thermodynamic state here.
        # Positivity safety, if enabled, handles this elsewhere.
        return

    # ----------------------------------------------------------------
    # Local nonequilibrium sensor.
    #
    # pressure scale: p = rho * kB*T / m
    # heat scale    : rho * (kB*T/m)^(3/2)
    #
    # This is purely local.  It does not use the reference solution and
    # it does not restore FP coefficients.
    # ----------------------------------------------------------------
    p = rv * KB * Tv / MASS
    if p <= 1.0e-300:
        p = 1.0e-300
    ...
    heat_scale = rv * vth2 * math.sqrt(vth2)
    if heat_scale <= 1.0e-300:
        heat_scale = 1.0e-300

    heat_norm = math.sqrt(qq) / heat_scale

    sensor = stress_norm + 0.25 * heat_norm

    if sensor < 0.0:
        sensor = 0.0
    if sensor > ESML_SENSOR_CAP:
        sensor = ESML_SENSOR_CAP

    active = sensor - ESML_SENSOR0
    if active < 0.0:
        active = 0.0

    # Smooth entropy limiter.  Stronger limiting for Gamma_i.
    lam_C = 1.0 / (1.0 + ESML_BETA_C * active)
    lam_G = 1.0 / (1.0 + ESML_BETA_G * active)

    if lam_C < ESML_LAMBDA_C_MIN:
        lam_C = ESML_LAMBDA_C_MIN
    if lam_G < ESML_LAMBDA_G_MIN:
        lam_G = ESML_LAMBDA_G_MIN

    # Apply to ML-predicted closure coefficients.
    for j in range(6):
        v = lam_C * c_C[idx, j]
        if v > ESML_COEFF_ABS_CLIP:
            v = ESML_COEFF_ABS_CLIP
        if v < -ESML_COEFF_ABS_CLIP:
            v = -ESML_COEFF_ABS_CLIP
        c_C[idx, j] = v

    for j in range(3):
        v = lam_G * c_Gamma[idx, j]
        if v > ESML_COEFF_ABS_CLIP:
            v = ESML_COEFF_ABS_CLIP
        if v < -ESML_COEFF_ABS_CLIP:
            v = -ESML_COEFF_ABS_CLIP
...
class CubicFPSolver:
    ...
    def step(self, step_count):
        ...
        if self.mode == 'PHYSICS':
            ...
            if SAFETY_ENABLE:
                safety_project_moments_kernel[blk_n, tp](self.rho, self.T, self.stress, self.heat)
            ...
            if SAFETY_ENABLE:
                X_cp = cp.where(cp.isfinite(X_cp), X_cp, 0.0)
            
            # Normalize & Forward Pass
            X_norm = (X_cp - self.cp_mean_in) / self.cp_scale_in
            if SAFETY_ENABLE:
                X_norm = cp.where(cp.isfinite(X_norm), X_norm, 0.0)
                X_norm = cp.clip(X_norm, -SAFETY_INPUT_CLIP_Z, SAFETY_INPUT_CLIP_Z)
            ...
            if SAFETY_ENABLE:
                Y_out = cp.where(cp.isfinite(Y_out), Y_out, 0.0)
                Y_out = cp.clip(Y_out, -SAFETY_OUTPUT_CLIP_Z, SAFETY_OUTPUT_CLIP_Z)
            Y_final = Y_out * self.cp_scale_out + self.cp_mean_out
            if SAFETY_ENABLE:
                Y_final = cp.where(cp.isfinite(Y_final), Y_final, 0.0)
                Y_final = cp.clip(Y_final, -SAFETY_COEFF_ABS_CLIP, SAFETY_COEFF_ABS_CLIP)
            ...
            if SAFETY_ENABLE:
                safety_damp_coefficients_kernel[blk_n, tp](self.c_C, self.c_Gamma)
            if SAFETY_ENABLE:
                safety_project_coefficients_kernel[blk_n, tp](self.c_C, self.c_Gamma, self.c_Beta, self.tau_arr, self.rho, self.T)
                if ESML_ENABLE and self.mode == 'ESML':
                    esml_entropy_limiter_kernel[blk_n, tp](
                        self.c_C, self.c_Gamma,
                        self.rho, self.T, self.stress, self.heat, self.tree_children
                    )
...
if __name__ == "__main__":
    ...
    # 2. Run ML
    print("\n--- Running ML Solver ---")
    solver_ml = CubicFPSolver(mode='ESML')
    for s in range(TOTAL_STEPS):
        solver_ml.step(s)
        if s % REPORT_INTERVAL == 0:
