...
RUN="$SYNC_ROOT/FPCode-stage53-boundary-$STAMP"
RESULT_ROOT="$RUN/results/riemann35_stage53/boundary_$STAMP"
BUNDLE="$RUN/STAGE53_BOUNDARY_RESULTS_$STAMP.zip"
...
JOB_RAW="$(
    sbatch --parsable \
        --job-name=fp-s53-boundary \
        --output="$RESULT_ROOT/logs/stage53-%j.out" \
        --error="$RESULT_ROOT/logs/stage53-%j.err" \
        --export="$EXPORTS" \
        "$RUN/riemann35_patch/stage53_boundary_realizability/run_unity_stage53.sbatch"
)"
JOB_ID="${JOB_RAW%%;*}"
