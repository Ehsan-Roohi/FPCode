...
COMMIT="${FP_STAGE54_COMMIT:-}"
SYNC_ROOT="${FP_STAGE54_SYNC_ROOT:-/project/pi_roohie_umass_edu/github_sync}"
PYTHON_BIN="${FP_STAGE54_PYTHON:-python}"

if [[ ! "$COMMIT" =~ ^[0-9a-f]{40}$ ]]; then
    echo "ERROR: FP_STAGE54_COMMIT must be the pinned 40-character commit SHA" >&2
    exit 2
fi
...
RUN="$SYNC_ROOT/FPCode-stage54-heatflux-$STAMP"
RESULT_ROOT="$RUN/results/riemann35_stage54/heat_flux_$STAMP"
BUNDLE="$RUN/STAGE54_HEAT_FLUX_RESULTS_$STAMP.zip"
...
EXPORTS="ALL,FP_STAGE54_RUN=$RUN,FP_STAGE54_RESULT_ROOT=$RESULT_ROOT,FP_STAGE54_BUNDLE=$BUNDLE,FP_STAGE54_PYTHON=$PYTHON_BIN"
ARRAY_RAW="$(
    sbatch --parsable \
        --array=0-5%6 \
        --job-name=fp-s54-q3 \
        ...
        --export="$EXPORTS" \
        "$RUN/riemann35_patch/stage54_heat_flux_third_order/run_unity_stage54_method.sbatch"
)"
ARRAY_JOB="${ARRAY_RAW%%;*}"
COLLECT_RAW="$(
    sbatch --parsable \
        --dependency="afterany:$ARRAY_JOB" \
        --job-name=fp-s54-pack \
        --output="$RESULT_ROOT/logs/collector-%j.out" \
        --error="$RESULT_ROOT/logs/collector-%j.err" \
        --export="$EXPORTS" \
        "$RUN/riemann35_patch/stage54_heat_flux_third_order/run_unity_stage54_collect.sbatch"
)"
