...
JOB=$(sbatch --parsable -p cpu -N 1 -n 1 -c 2 -t 08:00:00 --mem=12G -J m76-spatial-dvm -o "$OUT/logs/m76_%j.out" -e "$OUT/logs/m76_%j.err" --export=ALL,ROOT="$ROOT",OUT="$OUT",BUNDLE="$BUNDLE" "$ROOT/riemann35_patch/stage76_spatial_kinetic_dvm/run_stage76_job.sh")
